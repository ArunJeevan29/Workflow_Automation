// client/src/pages/employee/MyRequests.jsx
import React, { useState, useEffect } from 'react';
import { useNavigate } from 'react-router-dom';
import { Activity, Search, Filter, Clock, AlertCircle, FileText } from 'lucide-react';
...
export default function MyRequests() {
  ...
  const fetchData = async () => {
    try {
      // Fetch executions and filter for current user's requests
      const exResponse = await api.get('/executions');
      const allExecutions = exResponse.data.data || [];
      
      // Filter: user's executions (in_progress, pending only - NOT completed/failed/canceled)
  ...
  const getStatusIcon = (status) => {
    if (status === 'completed') return <Clock className="w-4 h-4 text-green-500" />;
    if (status === 'failed' || status === 'canceled') return <AlertCircle className="w-4 h-4 text-red-500" />;
  ...
  return (
    ...
    {/* My Requests - Only In Progress + Failed */}
    <div>
      <div className="mb-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold text-gray-900 tracking-tight flex items-center gap-2">
              <Activity className="w-5 h-5 text-[#58bfa1]" /> My Requests
            </h1>
            <p className="text-sm text-gray-500 mt-1">Track your in-progress and failed requests.</p>
          </div>
        ...
        {filteredRequests.length === 0 ? (
          <div className="flex flex-col items-center py-16 text-center">
            ...
            <p className="text-base font-semibold text-gray-700">
              {searchTerm ? 'No requests match your search.' : 'You have no in-progress or failed requests.'}
            </p>
            <p className="text-sm text-gray-400 mt-1">
              {searchTerm ? 'Try a different search term.' : 'Pending requests will appear here.'}
            </p>
          </div>
        ) : (
          <>
